...
# tests/test_analysis_group_router.py
import app.api.routers.analysis_group as r_ag
...
def test_query_sent_to_ES_via_run_search(client, monkeypatch):
    ...
    monkeypatch.setattr(r_ag, "run_search", mock_run_search, raising=True)
    req = {"query": {"multi_match": {"query":"exome","fields":["code","description"]}}, "size": 5}
    resp = client.post("/beta/analysis-group/_search", json=req)

    assert resp.status_code == 200
    assert captured["index"] == cfg.settings.INDEX_ANALYSIS_GROUP
...
def test_payload_received_from_run_search(client, monkeypatch):
    expected = {
        "took": 1,
        "timed_out": False,
        "hits": {"total": 1, "max_score": 1.0, "hits": [{"_source": {"code": "exome"}}]},
        "aggregations": {},
    }

    monkeypatch.setattr(r_ag, "run_search", lambda *a, **k: expected, raising=True)
    resp = client.post("/beta/analysis-group/_search", json={"query": {"match_all": {}}})
