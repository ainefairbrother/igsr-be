...
import app.api.routers.superpopulation as r_sp
...
def test_query_sent_to_ES_via_run_search(client, monkeypatch):
    ...
    monkeypatch.setattr(r_sp, "run_search", mock_run_search, raising=True)
    req = {"size": -1, "sort": ["display_order"]}
    resp = client.post("/beta/superpopulation/_search", json=req)
    
    print(resp.json())
    print(captured)

    assert resp.status_code == 200
    assert captured["index"] == cfg.settings.INDEX_SUPERPOPULATION
    assert captured["body"]["size"] == -1
    assert "sort" in captured["body"]
    assert "size_cap" in captured["kwargs"]
...
#     monkeypatch.setattr(r_sp, "run_search", lambda *a, **k: expected, raising=True)
