...
import app.api.routers.sitemap as r_sm
...
def test_query_sent_to_ES_via_run_search(client, monkeypatch):
    ...
    monkeypatch.setattr(r_sm, "run_search", mock_run_search, raising=True)
    req = {"query": {"match_phrase": {"query":"MAGE","fields":["title"]}}, "size": 5}
    resp = client.post("/sitemap/_search", json=req)

    assert resp.status_code == 200
    assert captured["index"] == cfg.settings.INDEX_SITEMAP
...
def test_payload_received_from_run_search(client, monkeypatch):
    expected = {
        "took": 1,
        "timed_out": False,
        "hits": {"total": 1, "max_score": 1.0, "hits": [{"_source": {"title": "mage_rnaseq"}}]},
        "aggregations": {},
    }

    monkeypatch.setattr(r_sm, "run_search", lambda *a, **k: expected, raising=True)
    resp = client.post("/sitemap/_search", json={"query": {"match_all": {}}})
